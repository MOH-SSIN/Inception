...
```bash
# From host
docker exec mariadb mysqldump -u YOUR_LOGIN -p$(cat secrets/db_password.txt) wordpress > backup.sql

# Restore
docker exec -i mariadb mysql -u YOUR_LOGIN -p$(cat secrets/db_password.txt) wordpress < backup.sql
```
